...
| 4 | **completa** | Fatti 4.1 e 4.2 (resize con rimozione dell'originale nostro e non di quello dell'utente, pulizia degli intermedi per pattern), 4.7 e 4.8 (screenshot come sorgente alternativa, verificato con grim reale su sway). Fatti anche 4.3 e 4.4 (formato adattivo per agente, con discesa nell'albero dei processi). Fatti anche 4.5 e 4.6 (ring buffer `--last`), 4.9 e 4.10 (annotazione via satty o swappy, ultima nella catena) e 4.11 (`--help` e `--doctor`). |
| 5 | **completa** | Fatti: bump a 2.0.0, CHANGELOG con sezione sulle verifiche mancanti, `SECURITY.md` e `.it.md` riscritti sulle mitigazioni reali, `Makefile` con `PREFIX` e `DESTDIR` provato, `docs/MANUAL_TESTING.md` e `.it.md`, `docs/ARCHITECTURE.*` riscritti sull'architettura a moduli, pacchetto `.deb` con `make deb`, `PKGBUILD`, `flake.nix`, workflow `release.yml` che allega gli artefatti. Il naming interno è allineato; la directory locale resta `paste-images-cli` per decisione dell'utente (nota in 5.8). |

Suite: 21 file di test, 244 casi, ShellCheck pulito su 51 file, gate dimensionale e di purezza dei moduli attivi.
...
Nota su 5.8: i riferimenti interni sono stati allineati (`.shellcheckrc`, header di `tests/test_framework.sh`) e l'URL di clone nei README, che puntava al segnaposto `github.com/user/`, è stato corretto. **La rinomina della directory locale non è stata eseguita**, per scelta esplicita dell'utente il 2026-07-30: il repo e ogni riferimento nel codice dicono già `cli-image-paste`, e il nome della directory di lavoro resta un disallineamento noto e accettato, non una svista.
