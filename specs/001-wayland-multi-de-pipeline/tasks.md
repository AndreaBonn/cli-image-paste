...
| 5 | **in corso** | Fatti: bump a 2.0.0, CHANGELOG con sezione sulle verifiche mancanti, `SECURITY.md` e `.it.md` riscritti sulle mitigazioni reali, `Makefile` con `PREFIX` e `DESTDIR` provato, `docs/MANUAL_TESTING.md` con la matrice del verificato e del non verificato. Mancano: `MANUAL_TESTING.it.md`, aggiornamento di `docs/ARCHITECTURE.*`, pacchetto `.deb`, `PKGBUILD`, `flake.nix`, allineamento del naming della directory locale. |

Suite: 20 file di test, 234 casi, ShellCheck pulito su 49 file, gate dimensionale e di purezza dei moduli attivi.
